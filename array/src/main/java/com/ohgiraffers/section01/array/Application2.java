package com.ohgiraffers.section01.array;

import java.util.Arrays;

public class Application2 {
    public static void main(String[] args) {

        /* 수업목표. 배열의 사용 방법을 익혀 배열을 사용할 수 있다. */
        /* 필기.
         * 배열의 사용 방법
         * 1. 배열의 선언
         * 2. 배열의 크기 할당
         * 3. 배열의 인덱스 공간에 값 대입
        */

        int[] iArr;
//        char cArr[];                        // 대괄호가 뒤에 붙어도 돌아가지만 권장되진 X

//        iArr = new int[];                     // 배열은 반드시 크기를 할당해 주어야 한다.
        iArr = new int[10];

        char[] cArr = new char[5];

        /* 설명. 참조자료형 변수 출력 결과 = toString()의 결과 -> 자료형@16진수 주소값*/
        System.out.println("iArr = " + iArr);
        System.out.println("cArr = " + cArr);
        System.out.println("iArr = " + iArr.toString());

        System.out.println("iArr의 10진수 주소 = " + iArr.hashCode());
        System.out.println("cArr의 10진수 주소 = " + cArr.hashCode());

        /* 설명. 위와 같이 할당하면 배열의 주소값이 저장되어 잇는 cArr의 값이 바뀌므로 배열을 지정해주는 주소값과는 달라지기 떄문에
         * 배열을 가르키는 것이 없어짐으로, 오래되면 자동으로 GC(garbage collector)에 의해서 OLD에 올려졌다가 삭제
         * 아래와 같이 cArr은 null과 동일하게 되고 이러한 경우에는 NullPointerException 오류 발생
         */
        cArr = null;
//        System.out.println("nill을 참조할 시: " + cArr.hashcode());


    }
}
