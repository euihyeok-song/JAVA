...
public class Application {
    public static void main(String[] args) {
        ...
        */
        ...
        /* 설명. 리터럴 값(참조 주소값 x)을 전달해서 메소들르 호출 시에는 서로 다른 지역 변수는 서로 영향 X */
        int num = 20;
        System.out.println("call by value 전: " + num);
        pt.testPrimitiveTypeparameter(num);                 // 리터럴 값에 의한 호출 -> 내부 class에서의 행위에 영향을 받지 X
        ...
        pt.testPrimitiveTypeArrayparameter(iArr);          // 참조 값에 의한 호출 -> 내부 class에서의 행위에 영향을 받음 O
        System.out.println("call by reference 후: " + Arrays.toString(iArr));
...
//        r1.calArea();
//        r1.calRound();
        pt.testClassTypeParameter(r1);                      // 참조 자료형에 의한 호출
...
//        pt.testClassTypeParameter(new String[]{"강감찬" , "낚시" , "독서"});
