...
public class Application {
     ...
     *  : 추상화, 캡슐화, 상속, 다형성을 적용하여 유지보수성을 고려한 응집력 높고 결합도 낮은(for 유지보수)
    ...
    public static void main(String[] args) {
        ...
        * */
        ...
        /* 필기.
         *  이렇게 변수들로만 관리 할 때 발생할 수 있는 문제점
         *  1. 많은 변수명들을 관리하기 힘들 수 있다.
         *  2. 메소드의 전달인자로 전달 할 값이 너무 많아 회원과 관련된 기능을 호출할 때 매개변수가 많아진다.(나쁜냄새)
         *  3. 메소드의 반환형으로 회원이라는 개념을 반환할 수 없다.(메소드의 반환형은 하나여야 하기 떄문에)
         */

        /* 설명. 두 명의 회원 객체를 만들어 각각 다른 이름을 부여해 보기*/
        Member member = new Member();                    // 객체는 DB에서의 한 행과 동일한 개념
        ...
        /* 필기. 얇은 복사(shallow copy) 가 일어남 => Member = m = returnValue가 가르키는 건 모두 동일 */
        Member returnValue = returnMember(member);                       // member 객체의 주소값을 넘김 ->
                                                                        // m(=member)의 주소가 return으로 옴
        System.out.println("개명 되었는지 확인: " + returnValue.name);


    }

    /* 설명. 전달인자가 넘어오면 이 주소값은 얇은 복사에 해당한다. */
