...
public class Application1 {
    public static void main(String[] args) {
        ...
        Integer boxingInt = (Integer)20;                   // boxing -> 리터럴 형태의 객체가 만들어짐
        Integer boxingInt2 = Integer.valueOf(intValue);

        /* 설명. Wrapper 클래스 자료형을 기본 자료형으로 변환할 수 있다. (언박싱, unboxing) */
        int unboxingValue = boxingInt.intValue();           // unboxing

        /* 설명. 기본자료형과 Wrapper 클래스는 자동으로 박싱 및 언박싱이 일어난다. (거의 이부분을 사용) */
        Integer autoBoxingInt = intValue;                   // auto-boxing
        int autoUnboxingInt = autoBoxingInt;                // auto-unboxing

        anythingMethod(10);

        /* 설명. Wrapper 클래스 주소 값 비교 - constant pool(상수풀)을 사용 => equals & identityHashCode 동일 */
...
//        System.out.println("integerTest 주소: " + integerTest.hashCode(integerTest));
//        System.out.println("integerTest2 주소: " + integerTest2.hashCode(integerTest2));
    ...
    public static void anythingMethod(Object obj){
        ...
        /* 설명. 10(int) -> Integer(오토박싱) -> Object(다형성) */
        /* 설명. 출력과정: obj.toString()에서(정적바인딩) Integer의 toString()이(동적바인딩) 실행됨*/
