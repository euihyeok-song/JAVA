...
//        ArrayList         // api에서 제공하는 list구조 ( 추상 클래스
         ...
         *   (단, 자식 클래스의 객체가 생성될 떄는 내부적으로 생성될 수 있다.)
         *   상속이 꼭 필요하다. (자식이 꼭 필요함)
         *
         * 필기.
         *  메소드의 헤드(head)만 있고 바디(body)부가 없는 메소드를 추상메소드라고 한다.
         *  쉽게 말해 불완전한 메소드라고 생각할 수 있다.
         *  불완전한 메소드를 하나라도 가진 클래스는 반드시 추상클래스가 되어야 한다.
         *  ex) public abstract void method();
         * */
//        Product product = new Product();                // Product는 abstract class(추상 클래스)이므로 객체 생성 X

        /* 설명. abstract 메소드를 구현한 온전한 자식 클래스는 인스턴스(객체)를 생성할 수 있다.*/
        SmartPhone smartPhone = new SmartPhone();
        Product smartPhone2 = new SmartPhone();          // 추상 클래스도 다형성 적용 O (타입은 물려줌)
